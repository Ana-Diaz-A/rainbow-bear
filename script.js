...
`;
const fireHearts = () => { // Creates and animates heart-shaped elements on a webpage
...
add(to([$BEAR_ARM_LEFT, $BEAR_ARM_RIGHT], { duration: SPEEDS.SWITCH, scale: 1, rotate: 0 })).
add(to([$BEAR_EAR_LEFT, $BEAR_EAR_RIGHT], { duration: SPEEDS.SWITCH, rotate: 0 }), 0).
add(to($BEAR_HEART, { duration: SPEEDS.SWITCH, scale: 1.1 }), 0).
add(to($BEAR_NOSE, { duration: SPEEDS.SWITCH, y: 0 }), 0).
add(to($BEAR_MOUTH, { duration: SPEEDS.SWITCH, y: 0, scaleY: 1 })).
add(to([$BEAR_CHEEK_LEFT, $BEAR_CHEEK_RIGHT], { duration: SPEEDS.SWITCH, opacity: 1, y: 0 }), 0).
add(to([$BEAR_EYE_LEFT, $BEAR_EYE_RIGHT], { duration: SPEEDS.SWITCH, clipPath: 'inset(0 0 0 0)', y: 0 }), 0).
add(to([$BEAR_PUPIL_LEFT, $BEAR_PUPIL_RIGHT], { duration: SPEEDS.SWITCH, y: 0 }), 0).
...
add(to([$BEAR_EYE_LEFT, $BEAR_EYE_RIGHT], { y: '-=0.4', duration: SPEEDS.BREATHING }), 0).
add(to([$BEAR_EAR_LEFT, $BEAR_EAR_RIGHT], { y: '-=0.4', duration: SPEEDS.BREATHING }), 0).
add(to($BEAR_MUZZLE, { y: '-=0.4', duration: SPEEDS.BREATHING }), 0);

let BLINKING_TL;
const blink = () => { // Creating a blinking animation effect on the eyes of a care bear. It uses the GSAP library to animate the scaleY property of the left and right eye elements, causing them to shrink and expand repeatedly.
  const DELAY = Math.floor(Math.random() * 5 - 1 + 1) + 1;
  BLINKING_TL = new timeline().add(to([$BEAR_EYE_LEFT, $BEAR_EYE_RIGHT], {
    delay: DELAY,
    scaleY: 0,
...
const tease = () => { // To animate the blinking of two elements representing the eyes of a care bear.
  BLINKING_TL.seek(0);
  BLINKING_TL.pause();
  to([$BEAR_EYE_RIGHT, $BEAR_EYE_LEFT], { duration: SPEEDS.SWITCH, clipPath: 'inset(0 0 0 0)' });
};

const sadden = () => { // To make the eyes of a care bear sad. It restarts the blinking timeline and animates the clip path of the bear's eyes to create a sad effect.
  BLINKING_TL.restart();
  to([$BEAR_EYE_RIGHT, $BEAR_EYE_LEFT], { clipPath: 'inset(50% 0 0 0)', duration: SPEEDS.SWITCH });
};
...
const start = () => { // Is called when a specific key is pressed. The function sets the STATE.FIRING variable to true, restarts a timeline animation, plays another timeline animation, pauses two other timeline animations, hides a DOM element, and calls another function.
...
const end = () => { // Is called when a specific key is released. It reverses a timeline animation, restarts another timeline animation, restarts a blinking timeline animation, displays a DOM element, and calls the sadden function.
...
if (letItRain) {
  ...
  for (let d = 0; d < Math.floor(Math.random() * 25); d++) {
    ...
    droplet.setAttribute( 'style', `--x: ${Math.floor(Math.random() * 100)}; --y: ${Math.floor( Math.random() * 100)}; --o: ${Math.random()}; --a: ${Math.random() + 0.5}; --d: ${Math.random() * 2 - 1}; --s: ${Math.random()}`);
